...
async function handleRegister() {
  try {
    const result = await axiosClient.post("/register", {
      email: email.value,
...
<template>
  <div class="card w-96 bg-base-100 shadow border">
    <div class="card-body">
      <h2 class="card-title text-2xl mb-3 justify-center">Register</h2>
      <form class="space-y-3" @submit.prevent="handleRegister">
        <label class="input input-bordered flex items-center gap-2">
          ...
          <input type="text" class="grow" placeholder="Email" :value="email" @change="(evt) => (email = evt.target.value)" />
        </label>
        <label class="input input-bordered flex items-center gap-2">
        ...
        <p>Already have account? <a class="text-primary cursor-pointer" @click="router.push('/')">Login</a></p>
      </form>
    </div>
  </div>
</template>
